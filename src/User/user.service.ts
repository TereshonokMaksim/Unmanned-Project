...
import { CODE_LENGTH, passwordCodes, transporter } from "../config";
...
export const UserService: UserServiceContract = {
    async login(credentials) {
        ...
        if (!isMatch) {
            throw new Error("What? It is not a passport!", {cause: "WRONG_CREDENTIALS"})
        };
    ...
    async register(credentials) {
        const user = await UserRepository.getUserByEmail(credentials.email);
        if (user) {
            throw new Error("I am already here.", {cause: "USER_EXISTS"})
        };
    ...
    async sendPasswordEmail(userEmail) {
        ...
        if (!userCheck){
            throw new Error("Who are you even talking about???", {cause: "USER_NOT_FOUND"})
        };
        ...
        transporter.sendMail({
            ...
            text: `Hello, you tried to restore password on our site. Here is restoration code link: http://${ENV.HOST}:${ENV.PORT}/users/password/${code}`
        });
    ...
    async checkCode(codeCheck, autoDelete, idCode?) {
        ...
        if (!possibleCodeArray.length){
            throw Error("What are you talking about?", {cause: "NOT_FOUND"})
        };
